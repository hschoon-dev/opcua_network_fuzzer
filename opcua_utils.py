...
target_apps = ["softing", "dotnetstd",
               "prosys", "unified", "kepware",
               "triangle", "ignition", "s2opc",
               "asyncua", "siemens_s7"]
...
def get_services_list(target_app):
    ...
    elif target_app == 'ignition': # lazy for ignition
        service_list = opcua_services_list_dotnetstd
    elif target_app == 's2opc':
        service_list = opcua_services_list_s2opc
    elif target_app == "asyncua": # lazy for asyncua
        service_list = opcua_services_list_dotnetstd
    elif target_app == "siemens_s7": # lazy for siemens_s7
        service_list = opcua_services_list_dotnetstd
...
def get_sanity_payload(target_app):
    ...
    if target_app == "siemens_s7":
        return SIEMENS_S7_MSG_READ
...
def close_session(sock, target_app, ses_info):
    try:
        close_session_payload, close_channel_payload = get_raw_close_session_messages(
            target_app)
        if target_app in ["prosys", "kepware", "softing", "unified", "triangle", "ignition", "s2opc", "asyncua", "siemens_s7"]:
            # close session
...
# Own Read requests, Author: Heiko Schoon
ASYNCUA_MSG_READ = b'MSGFo\x00\x00\x00\t\x00\x00\x00\r\x00\x00\x00\x04\x00\x00\x00\x04\x00\x00\x00\x01\x00w\x02\x01\x00\xe9\x03?\xea\xd1\x1a7\x08\xdc\x01CB\x0f\x00\x00\x00\x00\x00\xff\xff\xff\xff\x88\x13\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x03\x00\x00\x00\x02\x00\x00\x00\x01\x00\xb6-\r\x00\x00\x00\xff\xff\xff\xff\x00\x00\xff\xff\xff\xff\x01\x00\xb9-\r\x00\x00\x00\xff\xff\xff\xff\x00\x00\xff\xff\xff\xff'
SIEMENS_S7_MSG_READ = b'MSGFr\x00\x00\x00\x1d\x1d \x18\x01\x00\x00\x00\x04\x00\x00\x00\x04\x00\x00\x00\x01\x00w\x02\x02\x00\x00\x17\xe4\x13\x16\x19 \\\xd5\xa6\n\xdc\x01CB\x0f\x00\x00\x00\x00\x00\xff\xff\xff\xff\x88\x13\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x03\x00\x00\x00\x02\x00\x00\x00\x01\x00\xb6-\r\x00\x00\x00\xff\xff\xff\xff\x00\x00\xff\xff\xff\xff\x01\x00\xb9-\r\x00\x00\x00\xff\xff\xff\xff\x00\x00\xff\xff\xff\xff'
