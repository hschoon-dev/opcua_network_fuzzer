...
class OPCUA_Deep_Fuzzer(Fuzzer):
    def __init__(self, target_app_name, request_name, *args, **kwargs):
        ...
        if not os.path.exists("./logs"):
            os.makedirs("./logs")
        logging.basicConfig(handlers=[logging.FileHandler(filename=f'./logs/fuzzer_runtime{target_app_name}_{time.strftime("%m%d-%H%M%S")}.log',
                                                          encoding='utf-8', mode='w+')], format='%(asctime)s %(message)s', level=logging.INFO) # BUG
    ...
    def session_pre_send(self, target, fuzz_data_logger, session, sock):
        ...
        try:
            secure_channel_id, auth_id, sequence, secure_token_id = create_session(
                self.sock, self.target_app, session_timeout=59) # TODO: check if timeout changes anything
